...
module.exports = function(locals) {
  ...
  return locals.categories.reduce((result, category) => {
    ...
    category.date = page.date;
    category.type = page.type;
    category.layout = page.layout;
    if (page && page.content && page.content.length > 0) {
      category.content = page.content;
      if (page.position) {
        category.position = page.position;
      }
    }
    if (page && page.top !== undefined) {
      category.top = page.top;
    }
    if (page && page.recommend !== undefined) {
      category.recommend = page.recommend;
    }
    if (!category.length) {
      if (page && page.content && page.content.length > 0) {
